import type { EventTypeId } from "@/domain/marketplace";
...
export function saveCustomerEvent(event: CustomerEvent) {
  const all = (() => { try { return JSON.parse(localStorage.getItem(KEY) ?? "[]") as CustomerEvent[]; } catch { return []; } })();
...
export function readSelectedCustomerEventId(userId: string): string | null {
  try {
    const selected = JSON.parse(localStorage.getItem(SELECTED_KEY) ?? "null") as { userId: string; eventId: string } | null;
