...
export function saveDetailedQuotation(quotation: DetailedQuotation) {
  const all = (() => {
    try {
      return JSON.parse(localStorage.getItem(KEY) ?? "[]") as DetailedQuotation[];
    } catch {
      return [];
    }
  })();
...
export function readDetailedQuotations(providerId?: string): DetailedQuotation[] {
  try {
    const all = JSON.parse(localStorage.getItem(KEY) ?? "[]") as DetailedQuotation[];
    return providerId ? all.filter((quote) => quote.providerId === providerId) : all;
  } catch {
    return [];
  }
}
...
export function acceptDetailedQuotation(id: string) {
  ...
  const next = all.map((candidate) =>
    candidate.id === id ? { ...candidate, status: "accepted" as const } : candidate,
  );
